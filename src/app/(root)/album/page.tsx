...
const title = `Listen to New Songs Online Only on ${siteConfig.name}.`;
const description = `Listen to Latest and Trending songs online for free with ${siteConfig.name} anytime, anywhere. Download or listen to unlimited new & old songs online. Search from most trending, weekly top 15, movie songs, etc on ${siteConfig.name}.`;
...
export default async function AlbumsPage({ searchParams }: AlbumsPageProps) {
  ...
  const topAlbums = await getTopAlbums(page, 50);
